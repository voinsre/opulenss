...
import { MOCK_THREADS, MOCK_MESSAGES, MOCK_CLIENTS, MOCK_EVENTS } from "@/lib/data";
...
export default function TalentMessagesPage() {
    ...
    const getOtherParticipant = (thread: any) => {
        return MOCK_CLIENTS.find(c => c.id === thread.clientId);
    };
    ...
    return (
        <div className="flex h-full bg-offblack border-t border-white/10">
            {/* Thread List */}
                    ...
                    {threads.map(thread => {
                        ...
                        return (
                            <div
                                ...
                                className={`p-4 border-b border-white/5 cursor-pointer hover:bg-white/5 ${activeThreadId === thread.id ? 'bg-white/10' : ''}`}
                            >
                                <div className="flex justify-between mb-1">
                                    <span className="font-medium text-sm text-bone">{other?.name}</span>
                                    <span className="text-xs text-bone/40">10:30 AM</span>
                                </div>
                                <p className="text-xs text-emerald-400 mb-1">{event?.title}</p>
                                <p className="text-xs text-bone/60 truncate">Last message content...</p>
                            </div>
                        )
                    })}
                </div>
            ...
            {/* Chat Area */}
            <div className="flex-1 flex flex-col bg-offblack">
                {activeThread ? (
                    <>
                            ...
                            {messages.map(msg => {
                                ...
                                return (
                                    <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                                        <div className={`max-w-[70%] p-3 rounded-lg text-sm ${isMe ? 'bg-emerald-teal text-bone rounded-tr-none' : 'bg-white/10 border border-white/10 text-bone rounded-tl-none'
                                            }`}>
                                            {msg.content}
                                        </div>
                                    </div>
                        ...
                        <div className="p-4 bg-black/20 border-t border-white/10">
                            <div className="flex gap-2">
                                <Input
                                    ...
                                    className="bg-white/5 border-white/10 text-bone placeholder:text-bone/30"
                                />
                                <Button size="icon" className="bg-emerald-teal text-bone hover:bg-emerald-teal/90">
